...
public class DefaultGuestCheckoutCartCleanStrategyTest
{
	...
	public void testForGuestAbandonAnonymousCheckout() throws IOException, ServletException
	{
		...
		given(request.getRequestURL()).willReturn(new StringBuffer(
				"https://electronics.local:9002/electronicsonlinestorefront/electronics/en/Open-Catalogue/Cameras/Digital-Cameras/Digital-Compacts/PowerShot-A480/p/1934793"));
	...
	public void testForGuestInAnonymousCheckout() throws IOException, ServletException
	{
		...
		given(request.getRequestURL())
				.willReturn(new StringBuffer("https://electronics.local:9002/electronicsonlinestorefront/electronics/en/checkout/multi"));
		given(request.getSession()).willReturn(session);
	...
	public void testForGuestInAnonymousCheckoutForAjaxRequest() throws IOException, ServletException
	{
		...
		given(request.getRequestURL()).willReturn(
				new StringBuffer("https://electronics.local:9002/electronicsonlinestorefront/electronics/en/my-account/addressform"));
		given(request.getSession()).willReturn(session);
	...
	public void testWhetherURLContainsCheckoutPattern() throws IOException, ServletException
	{
		...
		given(request.getRequestURL())
				.willReturn(new StringBuffer("https://electronics.local:9002/electronicsonlinestorefront/electronics/en/checkou/multi"));
		assertEquals(Boolean.FALSE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));

		given(request.getRequestURL())
				.willReturn(new StringBuffer("electronics.local:9002/electronicsonlinestorefront/electronics/en/checkout/multi"));
		assertEquals(Boolean.FALSE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));
		...
		given(request.getRequestURL()).willReturn(new StringBuffer(
				"https://electronics.local:9002/electronicsonlinestorefront/electronics/en/checkout/multi/payment-method/add"));
		assertEquals(Boolean.TRUE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));

		given(request.getRequestURL()).willReturn(new StringBuffer(
				"https://electronics.local:9002/electronicsonlinestorefront/electronics/en/checkout/multi/delivery-address/add"));
		assertEquals(Boolean.TRUE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));
	}
	...
	{
		given(request.getRequestURL())
				.willReturn(new StringBuffer("https://electronics.local:9002/electronicsonlinestorefront/favicon"));
		assertEquals(Boolean.FALSE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));

		given(request.getRequestURL()).willReturn(new StringBuffer(
				"https://electronics.local:9002/electronicsonlinestorefront/_ui/responsive/theme-alpha/images/favicon.ico"));
		assertEquals(Boolean.TRUE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));

		given(request.getRequestURL())
				.willReturn(new StringBuffer("https://electronics.local:9002/electronicsonlinestorefront/favicon.ico"));
		assertEquals(Boolean.TRUE, Boolean.valueOf(guestCheckoutCartCleanStrategy.checkWhetherURLContainsSkipPattern(request)));
